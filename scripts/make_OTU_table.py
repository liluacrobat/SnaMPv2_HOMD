...
parser.add_argument('-c', '--collapsed_fp', help='collapsed OTU table file path', required=True)
...
if __name__ == "__main__":
    ...
    df.loc[:, 'taxonomy'] = df.index.map(lambda x: id_2_label[str(x)])
    df.to_csv(args.uncollapsed_fp, sep='\t', index=True, index_label='#OTU ID')
    collapsed_df = df.groupby('taxonomy').sum()
    collapsed_df.loc[:, '#OTU ID'] = ['OTU_{}'.format(x) for x in range(len(collapsed_df.index))]
    collapsed_df.loc[:, 'taxonomy'] = collapsed_df.index
    collapsed_df = collapsed_df[['#OTU ID'] + sample_ids + ['taxonomy']]
    collapsed_df.to_csv(args.collapsed_fp, sep='\t', index=False)
